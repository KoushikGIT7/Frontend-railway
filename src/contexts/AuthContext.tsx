...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const demoUsers = [
    { email: 'admin@railway.gov.in', password: 'admin123', name: 'Admin User', role: 'admin' as const },
    { email: 'drm@railway.gov.in', password: 'drm123', name: 'DRM User', role: 'drm' as const },
    { email: 'inspector@railway.gov.in', password: 'inspector123', name: 'Inspector User', role: 'inspector' as const },
  ...
  useEffect(() => {
    ...
    // If demo auth is explicitly enabled, skip Firebase listeners entirely
    if (useDemoAuth) {
  ...
  const login = async (email: string, password: string) => {
    // If demo mode is enabled, bypass Firebase entirely
    if (useDemoAuth) {
      ...
      if (demoUser) {
        ...
        setUser(demo);
        localStorage.setItem('railway_user', JSON.stringify(demo));
        return;
      }
