...
import { Package, Truck, CheckCircle, Clock, AlertTriangle, Search, Filter, Eye, Edit, RefreshCw } from 'lucide-react';
...
const OrderManagement: React.FC = () => {
  ...
  const loadOrders = async () => {
    setLoading(true);
    try {
      // Enhanced mock orders data for manufacturer
      const mockOrders: Order[] = [
        ...
        {
          id: 'ORD-2024-006',
          productName: 'High Voltage Cables',
          productId: 'HVC-11KV-500',
          quantity: 200,
          customer: 'South Central Railway',
          ...
          totalValue: 1200000,
          notes: 'Electrification project for new route'
        },
        {
          id: 'ORD-2024-007',
          productName: 'Track Monitoring Sensors',
          productId: 'TMS-PRO-100',
          quantity: 50,
          customer: 'North Western Railway',
          division: 'Jaipur Division',
          status: 'dispatched',
          priority: 'medium',
          orderDate: new Date('2024-01-22'),
          expectedDelivery: new Date('2024-02-18'),
          totalValue: 750000,
          notes: 'Smart track monitoring system installation'
        },
        {
          id: 'ORD-2024-008',
          productName: 'Rail Welding Equipment',
          productId: 'RWE-AUTO-25',
          quantity: 5,
          ...
          actualDelivery: new Date('2024-01-30'),
          totalValue: 2500000,
          notes: 'Automated welding equipment for track maintenance'
        },
        {
          id: 'ORD-2024-009',
          productName: 'Bridge Steel Components',
          productId: 'BSC-HD-200',
          quantity: 100,
          customer: 'Northeast Frontier Railway',
          ...
          totalValue: 4500000,
          notes: 'Bridge construction for new railway line'
        },
        {
          id: 'ORD-2024-010',
          productName: 'Communication Systems',
          productId: 'CS-DIG-75',
          quantity: 30,
          customer: 'Konkan Railway',
          ...
          totalValue: 1800000,
          notes: 'Digital communication upgrade for coastal route'
        }
      ];

      // Start with mock orders for UI structure
      let allOrders = [...mockOrders];
      
      // Try to load real orders from Firestore (fast access) - non-blocking
      const firestorePromise = fetch('/api/getProductsFast')
        ...
        .then(data => {
          if (data.products && data.products.length > 0) {
            // Convert Firestore products to Order format
            const firestoreOrders: Order[] = data.products.map((product: any) => ({
              id: `LIVE-ORD-${product.id}`,
              productName: product.name,
              productId: `LIVE-${product.id}`,
              quantity: product.quantity || 1,
              customer: product.customer || 'Railway Division',
              division: product.division || 'General Division',
              status: product.status === 'approved' ? 'approved' : 'pending',
              priority: product.priority === 'High' ? 'high' : product.priority === 'Medium' ? 'medium' : 'low',
              orderDate: product.orderDate ? new Date(product.orderDate) : new Date(),
              expectedDelivery: product.expectedDelivery ? new Date(product.expectedDelivery) : new Date(Date.now() + 30 * 24 * 60 * 60 * 1000),
              totalValue: product.totalValue || 0,
              notes: product.notes || 'Live order from system'
            }));
      ...
      // Set mock orders immediately for fast UI
      setOrders(allOrders);
      
      // Try to load real data in background
      try {
        const firestoreOrders = await firestorePromise;
        if (firestoreOrders.length > 0) {
          // Merge real orders with mock data
          const mergedOrders = [...mockOrders, ...firestoreOrders];
          setOrders(mergedOrders);
          console.log('✅ Loaded mock + live orders:', mergedOrders.length);
        }
      } catch (error) {
